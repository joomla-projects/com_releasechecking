...
class ContextController extends FormController
{
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_RELEASECHECKING_CONTEXT';
	...
	protected function allowAdd($data = [])
	{
		...
		// Access check.
		$access = $user->authorise('context.access', 'com_releasechecking');
		if (!$access)
		{
			return false;
		}
	...
	protected function allowEdit($data = [], $key = 'id')
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit', 'com_releasechecking.context.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('core.edit.own', 'com_releasechecking.context.' . $recordId))
				{
	...
	public function batch($model = null)
	{
		...
		// Preset the redirect
		$this->setRedirect(Route::_('index.php?option=com_releasechecking&view=contexts' . $this->getRedirectToListAppend(), false));
