...
class ActionsController extends AdminController
{
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_RELEASECHECKING_ACTIONS';
	...
	public function exportData()
	{
		...
		if ($user->authorise('action.export', 'com_releasechecking') && $user->authorise('core.export', 'com_releasechecking'))
		{
			// Get the input
			...
			if (UtilitiesArrayHelper::check($data))
			{
				...
				ReleasecheckingHelper::xls($data,'Actions_'.$date->format('jS_F_Y'),'Actions exported ('.$date->format('jS F, Y').')','actions');
			}
		}
		// Redirect to the list screen with error.
		$message = Text::_('COM_RELEASECHECKING_EXPORT_FAILED');
		$this->setRedirect(Route::_('index.php?option=com_releasechecking&view=actions', false), $message, 'error');
		return;
	}
	...
	public function importData()
	{
		...
		$user = Factory::getApplication()->getIdentity();
		if ($user->authorise('action.import', 'com_releasechecking') && $user->authorise('core.import', 'com_releasechecking'))
		{
			...
			if (ObjectHelper::check($headers))
			{
				...
				$message = Text::_('COM_RELEASECHECKING_IMPORT_SELECT_FILE_FOR_ACTIONS');
