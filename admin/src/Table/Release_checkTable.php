 ...
 * @since  1.6
 */
class Release_checkTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_releasechecking.release_check';
		...
		parent::__construct('#__releasechecking_release_check', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Release_check table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New release_check. A release_check created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_RELEASECHECKING_RELEASE_CHECK_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_RELEASECHECKING_RELEASE_CHECK_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_releasechecking.release_check.'.$this->id);
			$this->setRules($rules);
		}
