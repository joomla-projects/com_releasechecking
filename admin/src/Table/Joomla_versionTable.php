 ...
 */
namespace VDM\Component\Releasechecking\Administrator\Table;
 ...
 * @since  1.6
 */
class Joomla_versionTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_releasechecking.joomla_version';
		...
		parent::__construct('#__releasechecking_joomla_version', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Joomla_version table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New joomla_version. A joomla_version created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_RELEASECHECKING_JOOMLA_VERSION_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_RELEASECHECKING_JOOMLA_VERSION_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_releasechecking.joomla_version.'.$this->id);
			$this->setRules($rules);
		}
	...
	protected function _getAssetParentId(Table $table = null, $id = null)
	{
		...
		// load the releasechecking asset
		$assets->loadByName('com_releasechecking');
