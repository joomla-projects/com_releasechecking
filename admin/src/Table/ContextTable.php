 ...
 * @since  1.6
 */
class ContextTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_releasechecking.context';
		...
		parent::__construct('#__releasechecking_context', 'id', $db, $dispatcher);
	}
	...
	public function store($updateNulls = false)
	{
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_RELEASECHECKING_CONTEXT_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_RELEASECHECKING_CONTEXT_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_releasechecking.context.'.$this->id);
			$this->setRules($rules);
		}
