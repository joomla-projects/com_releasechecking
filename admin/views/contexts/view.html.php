...
class Release_checkingViewContexts extends HtmlView
{
	...
	function display($tpl = null)
	{
		if ($this->getLayout() !== 'modal')
		{
			...
			Release_checkingHelper::addSubmenu('contexts');
		}
	...
	{
		JToolBarHelper::title(JText::_('COM_RELEASE_CHECKING_CONTEXTS'), 'joomla');
		JHtmlSidebar::setAction('index.php?option=com_release_checking&view=contexts');
		JFormHelper::addFieldPath(JPATH_COMPONENT . '/models/fields');

		if ($this->canCreate)
		{
			JToolBarHelper::addNew('context.add');
		}
		...
		if (Release_checkingHelper::checkArray($this->items))
		{
			if ($this->canEdit)
			{
				JToolBarHelper::editList('context.edit');
			}

			if ($this->canState)
			{
				JToolBarHelper::publishList('contexts.publish');
				JToolBarHelper::unpublishList('contexts.unpublish');
				JToolBarHelper::archiveList('contexts.archive');

				if ($this->canDo->get('core.admin'))
				{
					JToolBarHelper::checkin('contexts.checkin');
				}
			}
			...
			{
				JToolbarHelper::trash('contexts.trash');
			}

			if ($this->canDo->get('core.export') && $this->canDo->get('context.export'))
			{
				JToolBarHelper::custom('contexts.exportData', 'download', '', 'COM_RELEASE_CHECKING_EXPORT_DATA', true);
			}
		}

		if ($this->canDo->get('core.import') && $this->canDo->get('context.import'))
		{
			JToolBarHelper::custom('contexts.importData', 'upload', '', 'COM_RELEASE_CHECKING_IMPORT_DATA', false);
		}
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_('COM_RELEASE_CHECKING_CONTEXTS'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_release_checking/assets/css/contexts.css", (Release_checkingHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
	}
